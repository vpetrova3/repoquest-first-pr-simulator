...
const STORAGE_KEY_THEME = "repoquest:theme";

const HLJS_THEMES = {
  dark: "https://cdn.jsdelivr.net/gh/highlightjs/cdn-release@11.10.0/build/styles/github-dark.min.css",
  light: "https://cdn.jsdelivr.net/gh/highlightjs/cdn-release@11.10.0/build/styles/github.min.css",
};
...
function applyInitialTheme() {
  const stored = safeStorageGet(STORAGE_KEY_THEME);
  ...
  if (window.matchMedia && !stored) {
    window.matchMedia("(prefers-color-scheme: light)").addEventListener?.("change", (event) => {
      if (safeStorageGet(STORAGE_KEY_THEME)) return;
...
function setTheme(theme, { persist = true } = {}) {
  ...
  if (metaTheme) metaTheme.setAttribute("content", next === "light" ? "#f6f7f4" : "#0a0f12");
  const hljsLink = document.getElementById("hljsThemeLink");
  if (hljsLink && HLJS_THEMES[next]) hljsLink.setAttribute("href", HLJS_THEMES[next]);
  if (persist) safeStorageSet(STORAGE_KEY_THEME, next);
}
...
function bindEvents() {
  ...
  if (selectors.downloadPlanButton) {
    selectors.downloadPlanButton.addEventListener("click", () => downloadPrPlan(state.analysis));
  }
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (cmd && event.shiftKey && event.key.toLowerCase() === "l") {
      event.preventDefault();
      toggleTheme();
    } else if (cmd && event.key.toLowerCase() === "k") {
      event.preventDefault();
...
function buildPrPlanMarkdown(analysis) {
  const { repo, firstPr } = analysis;
  const date = new Date().toISOString().slice(0, 10);
  return `# ${firstPr.title}

> First-PR plan for [${repo.fullName || repo.name}](${repo.url || "#"})
> Generated by RepoQuest on ${date}

**Risk:** ${firstPr.risk}
**Files:** ${firstPr.files.map((f) => `\`${f}\``).join(", ")}

## Implementation steps

${firstPr.steps.map((step, i) => `${i + 1}. ${step}`).join("\n")}

## Test plan

${firstPr.tests.map((step, i) => `${i + 1}. ${step}`).join("\n")}

## Reviewer checklist

${firstPr.checklist.map((item) => `- [ ] ${item}`).join("\n")}

---

*Use this as the body of your draft pull request. Trim or expand sections as the change requires.*
`;
}

function downloadPrPlan(analysis) {
  if (!analysis?.firstPr) return;
  const repoSlug = (analysis.repo.fullName || analysis.repo.name || "repo").replace(/[^a-z0-9._-]+/gi, "-");
  const filename = `${repoSlug}-first-pr.md`;
  const blob = new Blob([buildPrPlanMarkdown(analysis)], { type: "text/markdown;charset=utf-8" });
  ...
  setStatus(`Downloaded ${filename}`, "check");
}
